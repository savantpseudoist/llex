...
class EditorApp:
    ...
    def _build_toolbar(self) -> None:
        ...
        for text, command in (
            ...
            ("Text Color", self._apply_text_color),
            ("Highlight", self._apply_highlight),
        ):
    ...
    def _build_status_bar(self) -> None:
        ...
        self.status_bar = tk.Label(self.status_frame, text="Ready", anchor=tk.W)
        self.status_bar.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.word_count_label = tk.Label(self.status_frame, text="0 words | 0 chars", anchor=tk.E)
        self.word_count_label.pack(side=tk.RIGHT, padx=4)
    ...
    def _apply_text_color(self) -> None:
        selection = self._selection_range()
        if not selection:
            self._update_status("Select text to colorize.")
            return
        ...
        if color:
            ...
            self._update_status(f"Applied text color {color}.")

    def _apply_highlight(self) -> None:
        selection = self._selection_range()
        if not selection:
            self._update_status("Select text to highlight.")
            return

        color = colorchooser.askcolor(title="Choose Highlight Color", color="#fff5b1")[1]
        if color:
            tag_name = f"bg_{color}"
            self.text_widget.tag_configure(tag_name, background=color)
            start, end = selection
            self.text_widget.tag_add(tag_name, start, end)
            self._update_status(f"Applied highlight {color}.")
    ...
    def _on_text_modified(self, event=None) -> None:
        if self.text_widget.edit_modified():
            text_content = self.text_widget.get("1.0", tk.END).rstrip("\n")
