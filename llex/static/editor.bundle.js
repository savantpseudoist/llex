...
${nextLine.slice(indentLevel + 2)}`;
  ...
  // llex/static/editor.js
  var ScaffoldMark = Mark2.create({
    name: "scaffold",
    keepOnSplit: false,
    addAttributes() {
      return {
  ...
  var CustomDocument = index_default.extend({
    content: "page+"
  });
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    const editor2 = new Editor({
      ...
      editorProps: {
        ...
        handlePaste: (view, event, slice2) => {
          if (slice2.content.size > 8e3) {
            const confirmPaste = confirm("You are pasting a large amount of text (" + slice2.content.size + " chars). This may cause a 'ripple' effect as blocks are reflowed into pages. Proceed?");
            if (!confirmPaste) {
              return true;
            }
          }
      ...
      onUpdate: ({ editor: editor3 }) => {
        ...
        requestAnimationFrame(() => {
          ...
          for (let i = 0; i < domPages.length; i++) {
            ...
            const pageNode = editor3.state.doc.child(i);
            if (domPage.scrollHeight > domPage.clientHeight + 1) {
              try {
                ...
                editor3.commands.command(({ tr: tr2, dispatch }) => {
                  if (dispatch) {
                    const blockSlice = tr2.doc.slice(lastChildStart, lastChildStart + lastChild.nodeSize);
                    const nextPageIndex = i + 1;
                    if (nextPageIndex < domPages.length) {
                      const nextPos = offset + pageNode.nodeSize;
                      tr2.insert(nextPos + 1, blockSlice.content);
                      tr2.delete(lastChildStart, lastChildStart + lastChild.nodeSize);
                    } else {
                      const newPage = editor3.schema.nodes.page.create(null, blockSlice.content.content);
                      tr2.insert(offset + pageNode.nodeSize, newPage);
                      tr2.delete(lastChildStart, lastChildStart + lastChild.nodeSize);
                    }
                  }
                  return true;
      ...
      onSelectionUpdate: ({ editor: editor3 }) => {
        updateFormatState();
      }
    });
    ...
    if (menuSave) menuSave.addEventListener("click", async () => {
      ...
      try {
        await fetch("/api/save", {
          ...
          body: JSON.stringify({ html: editor2.getHTML() })
        });
        menuSave.innerText = "Saved!";
        setTimeout(() => {
          menuSave.innerText = btnText;
        }, 2e3);
      } catch (err) {
    ...
    if (btnUndo) btnUndo.addEventListener("click", () => editor2.chain().focus().undo().run());
    if (btnRedo) btnRedo.addEventListener("click", () => editor2.chain().focus().redo().run());
    if (btnBold) btnBold.addEventListener("click", () => editor2.chain().focus().toggleBold().run());
    if (btnItalic) btnItalic.addEventListener("click", () => editor2.chain().focus().toggleItalic().run());
    if (btnUnderline) btnUnderline.addEventListener("click", () => editor2.chain().focus().toggleUnderline().run());
    if (btnStrikethrough) btnStrikethrough.addEventListener("click", () => editor2.chain().focus().toggleStrike().run());
    if (btnAlignLeft) btnAlignLeft.addEventListener("click", () => editor2.chain().focus().setTextAlign("left").run());
    if (btnAlignCenter) btnAlignCenter.addEventListener("click", () => editor2.chain().focus().setTextAlign("center").run());
    if (btnAlignRight) btnAlignRight.addEventListener("click", () => editor2.chain().focus().setTextAlign("right").run());
    if (btnAlignJustify) btnAlignJustify.addEventListener("click", () => editor2.chain().focus().setTextAlign("justify").run());
    if (styleDropdown) styleDropdown.addEventListener("change", (e) => {
      const val = e.target.value;
      if (val === "p") {
        editor2.chain().focus().setParagraph().run();
      } else if (val.startsWith("h")) {
        const level = parseInt(val.charAt(1));
        editor2.chain().focus().setHeading({ level }).run();
      }
    });
    if (btnBullet) btnBullet.addEventListener("click", () => editor2.chain().focus().toggleBulletList().run());
    if (btnNumber) btnNumber.addEventListener("click", () => editor2.chain().focus().toggleOrderedList().run());
    ...
    if (editorContainer && contextMenu) {
      editorContainer.addEventListener("contextmenu", (e) => {
        ...
        e.preventDefault();
        const hasSelection2 = !editor2.state.selection.empty;
        if (hasSelection2) {
          ...
          if (ffSelect && editor2.getAttributes("textStyle").fontFamily) {
            ffSelect.value = editor2.getAttributes("textStyle").fontFamily;
          }
          const activeMarks = editor2.state.selection.$from.marks().map((m) => m.type.name);
      ...
      editor2.view.dom.addEventListener("keydown", () => {
        contextMenu.style.display = "none";
        ...
        contextMenuStandard.addEventListener("click", async (e) => {
          ...
          if (action === "cut") {
            ...
            try {
              ...
              editor2.chain().focus().insertContent(text).run();
            } catch (err) {
      ...
      contextMenu.addEventListener("click", (e) => {
        ...
        switch (action) {
          ...
          case "decrease-font":
            break;
          case "clear-format":
            editor2.chain().focus().unsetAllMarks().clearNodes().run();
            break;
          case "bullet-list":
          ...
          case "add-scaffold":
            ...
            if (comment) {
              ...
              editor2.view.dispatch(editor2.state.tr.addMark(
                editor2.state.selection.from,
                editor2.state.selection.to,
                editor2.schema.marks.scaffold.create({ id, comment })
              ));
            }
            contextMenu.style.display = "none";
            if (contextMenuStandard) contextMenuStandard.style.display = "none";
      ...
      contextMenu.addEventListener("change", (e) => {
        ...
        if (action === "font-family") {
          editor2.chain().focus().setFontFamily(select.value).run();
      ...
      outlineTimeout = setTimeout(() => {
        ...
        editor2.state.doc.descendants((node, pos) => {
          if (node.type.name === "heading") {
            ...
            if (level >= 3) {
              items.push({ text: node.textContent, level, pos });
            }
          }
        });
        ...
        items.forEach((item) => {
          ...
          div.onclick = () => {
            editor2.chain().focus().setTextSelection(item.pos).scrollIntoView().run();
          };
    ...
    function updateFormatState() {
      if (btnBold) {
        if (editor2.isActive("bold")) btnBold.classList.add("active");
        else btnBold.classList.remove("active");
      }
      if (btnItalic) {
        if (editor2.isActive("italic")) btnItalic.classList.add("active");
        else btnItalic.classList.remove("active");
      }
      if (btnUnderline) {
        if (editor2.isActive("underline")) btnUnderline.classList.add("active");
        else btnUnderline.classList.remove("active");
      }
      if (btnStrikethrough) {
        if (editor2.isActive("strike")) btnStrikethrough.classList.add("active");
        else btnStrikethrough.classList.remove("active");
      }
      if (btnAlignLeft) {
        if (editor2.isActive({ textAlign: "left" })) btnAlignLeft.classList.add("active");
        else btnAlignLeft.classList.remove("active");
      }
      if (btnAlignCenter) {
        if (editor2.isActive({ textAlign: "center" })) btnAlignCenter.classList.add("active");
        else btnAlignCenter.classList.remove("active");
      }
      if (btnAlignRight) {
        if (editor2.isActive({ textAlign: "right" })) btnAlignRight.classList.add("active");
        else btnAlignRight.classList.remove("active");
      }
      if (btnAlignJustify) {
        if (editor2.isActive({ textAlign: "justify" })) btnAlignJustify.classList.add("active");
        else btnAlignJustify.classList.remove("active");
      }
      if (styleDropdown) {
        let currentStyle = "p";
        for (let i = 1; i <= 6; i++) {
          if (editor2.isActive("heading", { level: i })) {
            currentStyle = "h" + i;
            break;
          }
        }
      ...
      if (btnBullet) {
        if (editor2.isActive("bulletList")) btnBullet.classList.add("active");
        else btnBullet.classList.remove("active");
      }
      if (btnNumber) {
        if (editor2.isActive("orderedList")) btnNumber.classList.add("active");
          ...
          if (mark && mark.attrs.id) {
            ...
            if (existing) {
              ...
              payloadScaffolds.push({
                id: Number(mark.attrs.id),
                // Ensure it matches backend Pydantic validator integer
        ...
        try {
          ...
          if (data.status === "success") {
            ...
            payloadScaffolds.forEach((scaffold) => {
              ...
              if (resultObj) {
                tr2.removeMark(scaffold.start, scaffold.end, editor.schema.marks.scaffold);
                tr2.insertText(resultObj.text, scaffold.start, scaffold.end);
              }
            });
